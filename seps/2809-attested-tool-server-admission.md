...
5. the key is approved for the asserted `clearance`, compared against the
   canonical level name so that an alias cannot widen a key's scope;
...
([SEP-3004](https://omcp.tech/seps/3004-tamper-evident-audit-record-contract)),
...
A trust root binds each key to the set of clearance levels it may sign for. That
set is an **explicit list of canonical level names**, not a ceiling: a ceiling
and a list differ for non-contiguous approvals, and the explicit list is the
unambiguous reading of rule 5.
...
`unsupported_version` (0), `not_mcp_server` (1), `unsigned` (2),
`signer_not_trusted` (3), `signer_expired` (4), `signer_not_approved` (5),
`bad_signature` (6), `below_required` (7), `host_not_bound` (8), and
`tool_not_admitted` (9, tool authorization).

`unsupported_version` (0) reports the rejection required by _Compatibility,
versioning, negotiation_: it precedes the numbered rules, because a document
whose version a verifier does not understand cannot be evaluated against them.
...
principles](https://omcp.tech/community/design-principles):
...
A reference implementation, a JSON Schema for the document, and conformance
vectors ship in this repository at
[`extensions/atsa`](https://github.com/enclawed/omcp/tree/main/extensions/atsa).

Seventeen vectors cover every host verification rule and every reason in the
error registry, committed as data together with the trust root they verify
against, so an implementation in any language can be checked against the same
cases. The signing keys derive from fixed seeds, so the vectors are
byte-for-byte reproducible. They run with `npm test`, without network access,
a browser, or any key material supplied by the reader.

The reference chapter is published at
[Attested Tool-Server Admission](https://omcp.tech/extensions/attestation/overview).

Implementing the specification surfaced two under-specified points, both now
resolved in the text above: rejecting an unsupported document version had no
registered reason code, and the expression of key approval was unstated.
