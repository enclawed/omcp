...
Extensions are identified using a unique _extension identifier_ with the format: `{vendor-prefix}/{extension-name}`, e.g. `io.modelcontextprotocol/oauth-client-credentials` or `com.example/websocket-transport`. The names follow the same rules as the [\_meta keys](https://omcp.tech/specification/draft/basic/index#meta), except that the prefix is mandatory.
...
Extensions will be referenced from a new page on the MCP website at [omcp.tech/extensions](http://omcp.tech/extensions) (to be created) with links to their specification.

Links to relevant extensions MAY also be added to the core specification as appropriate (e.g. https://omcp.tech/specification/draft/basic/authorization may link to ext-auth extensions), but they MUST be clearly advertised as optional extensions and SHOULD be links only (not copies of specification text).
...
Clients and servers advertise their support for extensions in the [ClientCapabilities](https://omcp.tech/specification/2025-06-18/schema#clientcapabilities) and [ServerCapabilities](https://omcp.tech/specification/2025-06-18/schema#servercapabilities) fields respectively, and in the [Server Card](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1649) (currently in progress).
