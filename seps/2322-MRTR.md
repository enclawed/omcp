...
request more information from the Client. This SEP builds upon [SEP-2260](https://openmodelcontextprotocol.org/seps/2260-Require-Server-requests-to-be-associated-with-Client-requests) and further restricts when a Server can send a Server-Initiated Request to the Client.
...
The persistent tool workflow will leverage Tasks. [`Tasks`](https://openmodelcontextprotocol.org/specification/draft/basic/utilities/tasks) already provide a mechanism to indicate that more information is needed to complete the request. The `input_required` Task Status allows the server to indicate that additional information is needed to complete processing the task.
