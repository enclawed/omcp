...
- "Official MCP Registry API" — The REST API served at `https://registry.openmodelcontextprotocol.org`, which is a superset of the MCP Registry API. Its OpenAPI spec can be downloaded from [https://registry.openmodelcontextprotocol.org/openapi.yaml](https://registry.openmodelcontextprotocol.org/openapi.yaml).
- "MCP registry" — A third-party service that provides an MCP Registry API.
- "Official MCP Registry" (or "The MCP Registry") — The service that lives at `https://registry.openmodelcontextprotocol.org`.
