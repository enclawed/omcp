...
Between January 23 and February 23, SDK maintainers can adopt the tests and set
up GitHub issue tracking with the standardized labels defined below.
...
2. Opening an issue in the [enclawed/omcp](https://github.com/enclawed/omcp) repository with supporting evidence
3. Passing automated conformance testing

Tiers follow the test results. The conformance suite is public and anyone can run it, so an
advancement claim is verified by reproducing it — not granted by anybody's approval.
